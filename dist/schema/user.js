...
exports.userUpdateById = exports.userCreateOne = exports.userFindById = undefined;
...
const validatedInputResolver = next => async rp => {
  ...
  if (email) {
    if (!email.match(emailRegex)) throw new _apolloServer.ApolloError(' email is not valid ');
  }

  if (no_hp) {
    if (!no_hp.match(handPhoneRegex)) throw new _apolloServer.ApolloError(' phone number is not valid');
  }

  return next(rp);
};

const userFindById = exports.userFindById = _user.UserTC.mongooseResolvers.findById();

const userCreateOne = exports.userCreateOne = _user.UserTC.mongooseResolvers.createOne().wrapResolve(validatedInputResolver);

const userUpdateById = exports.userUpdateById = _user.UserTC.mongooseResolvers.updateById().wrapResolve(validatedInputResolver); // const AmazonCognitoIdentity = require('amazon-cognito-identity-js');
