...
const handPhoneExp =
  /^((\+62 8\d{2}([ -])|08\d{2}([ -]?)|\+628\d{2})\d{4}(\3\4)\d{2,5})$/i;
...
const validatedInputResolver = (next) => async (rp) => {
  ...
  if (email) {
    if (!email.match(emailRegex)) throw new ApolloError(' email is not valid ');
  }
  if (no_hp) {
    if (!no_hp.match(handPhoneRegex))
      throw new ApolloError(' phone number is not valid');
  }
  return next(rp);
};
...
export const userCreateOne = UserTC.mongooseResolvers
  .createOne()
  .wrapResolve(validatedInputResolver);
export const userUpdateById = UserTC.mongooseResolvers
  .updateById()
  .wrapResolve(validatedInputResolver);
